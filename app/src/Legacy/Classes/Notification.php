...
class Notification
{
    ...
    public static function get(array $args = []): array
    {
        ...
        if (isset($get[0])) {
            ...
            self::fill($get);
            if (!self::hasContent($get)) {
                $toRemove[] = $get['id'];
                $get = [];
            }
        }
        // TODO: Removals as background job
        if ($toRemove !== []) {
            $toRemoveIn = implode(',', $toRemove);
            DB::queryExecute(
                <<<SQL
                DELETE FROM {$tables['notifications']} WHERE notification_id IN ({$toRemoveIn});
                SQL
            );
        }
