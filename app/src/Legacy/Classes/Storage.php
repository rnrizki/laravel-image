...
class Storage
{
    ...
    public static function uploadFiles(
        ...
        foreach ($files as $k => $v) {
            $source_file = $v['file'];
            if (in_array($storage['api_type'], ['s3', 's3compatible', 'b2', 'azure', 'oss'], true)) {
        ...
        if (in_array($storage['api_type'], ['ftp', 'sftp'], true) && is_object($API)) {
            $API->close();
        }
    ...
    public static function deleteObject(string $key, array $storage): void
    {
        ...
        switch (StorageApis::getApiType((int) $storage['api_id'])) {
            ...
            case 'b2':
                /** @var Client $API */
                try {
                    $API->deleteFileFromArray(
                        [
                            'BucketId' => $API->getBucketFromName($storage['bucket'])->getId(),
                            'BucketName' => $storage['bucket'],
                            'FileName' => $key,
                        ]
                    );
    ...
    public static function requireAPI(array $storage): object
    {
        ...
        switch ($api_type) {
            ...
            case 'b2':
                try {
                    ...
                    return new Client(
                        ...
                        [
                            'cacheParentDir' => PATH_APP_CACHE . 'vendor/b2-legacy/',
                        ]
                    );
                } catch (Exception $e) {
            ...
            case 'gcloud':
                ...
                $config = json_decode(trim($storage['secret']), true);
                if (json_last_error() !== JSON_ERROR_NONE) {
                    throw new Exception('Invalid private key JSON format');
                }
    ...
    public static function getThrowableMessage(Throwable $throwable): string
    {
        ...
        if ($throwable instanceof ServiceException) {
            $message = $throwable->getErrorText();
        }
        if ($throwable instanceof OssException
            && ($throwable->getErrorMessage() ?? '') !== ''
        ) {
            $message = $throwable->getErrorCode()
                . ': '
                . $throwable->getErrorMessage();
        }
    ...
    protected static function formatValues(array &$values, string $junk = 'keep'): void
    {
        ...
        if (isset($values['api_id'])
            && in_array(StorageApis::getApiType((int) $values['api_id']), ['ftp', 'sftp'], true)
            && isset($values['bucket'])
        ) {
