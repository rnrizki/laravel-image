        ...
        foreach ($queues as $k => $storage_queue) {
            ...
            switch ($storage['api_type']) {
                ...
                case 's3compatible':
                    ...
                    try {
                        // if (str_ends_with($storage['server'], '.backblazeb2.com')
                        //     && (bool) (env()['CHEVERETO_X_S3COMPATIBLE_HEADER_CONTENT_MD5'] ?? 1)
                        // ) {
                        // https://www.backblaze.com/docs/cloud-storage-s3-compatible-api
                        // $StorageAPI->getHandlerList()
                        //     ->appendBuild(
                        //         Middleware::mapRequest(static function (RequestInterface $request) {
                        //             $contentMd5 = base64_encode(
                        //                 Utils::hash($request->getBody(), 'md5', true)
                        //             );

                        //             return $request
                        //                 ->withoutHeader('x-amz-checksum-crc32')
                        //                 ->withoutHeader('x-amz-checksum-crc32c')
                        //                 ->withoutHeader('x-amz-checksum-crc64nvme')
                        //                 ->withoutHeader('x-amz-checksum-sha1')
                        //                 ->withoutHeader('x-amz-checksum-sha256')
                        //                 ->withoutHeader('x-amz-checksum-algorithm')
                        //                 ->withoutHeader('x-amz-checksum-mode')
                ...
                case 'gcloud':
                case 'ftp':
                    if ($storage['api_type'] === 'b2') {
                        $BucketId = $StorageAPI->getBucketFromName($storage['bucket'])->getId();
                    }
                    foreach ($files as $k => $v) { // No batch operation here
                        ...
                        switch ($storage['api_type']) {
                            ...
                            case 'b2':
                                try {
                                    $StorageAPI->deleteFileFromArray(
                                        [
                                            'BucketId' => $BucketId, // @phpstan-ignore-line
                                            'BucketName' => $storage['bucket'],
                                            'FileName' => $v['key'],
                                        ]
                                    );
                                } catch (\obregonco\B2\Exceptions\NotFoundException) {
