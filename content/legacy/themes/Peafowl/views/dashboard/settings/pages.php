...
<?php if (Handler::var('settings_pages')['doing'] !== 'listing') {
    ...
    <div class="input-label">
        ...
        <div class="c9 phablet-c1"><input type="text" name="page_title" id="page_title" class="text-input" value="<?php echo safe_html(get_page_val('page_title')); ?>" required placeholder="<?php _se('Page title'); ?>"></div>
    ...
    <div id="page-type-combo">
        ...
        <div data-combo-value="internal" class="switch-combo phablet-c1<?php if (!$page_internal_combo_visible) {
            ...
            <div class="input-label">
                ...
                <div class="c5 phablet-c1"><select type="text" name="page_is_link_visible" id="page_is_link_visible" class="text-input" <?php echo $page_internal_combo_visible ? 'required' : 'data-required'; ?>>
                        <?php
                        echo get_select_options_html([1 => _s('Visible page'), 0 => _s('Hidden page')], (int) get_page_val('page_is_link_visible', post() !== [] ? 'POST' : 'DB')); ?>
                    </select></div>
            ...
            <?php if(Config::enabled()->phpPages()) { ?>
            <div class="input-label">
        ...
        _se('A %f file relative to %s', ['%f' => !Config::enabled()->phpPages() ? 'HTML' : 'PHP', '%s' => $pages_visible_path]); ?></div>
            ...
            <div class="input-label">
                ...
                <div class="c9 phablet-c1"><input type="text" name="page_keywords" id="page_keywords" class="text-input" value="<?php echo safe_html(get_page_val('page_keywords')); ?>"></div>
            ...
            <div class="input-label">
                ...
                <div class="c9 phablet-c1"><textarea type="text" name="page_description" id="page_description" class="text-input resize-vertical r2"><?php echo safe_html(get_page_val('page_description')); ?></textarea></div>
...
if(Config::enabled()->phpPages()) {
    if (Handler::var('settings_pages')['doing'] == 'add') {
        $page_path_absolute = get_page_val('page_file_path_absolute');
        ...
        $pageCode = get_page_val('page_code');
    } else { // edit
        $page_path_absolute = get_page_val('page_file_path_absolute');
        $page_write_path = get_page_val('page_file_path');
        $page_write_path = Page::getPath($page_write_path);
        if (!get_page_val('page_file_path') || !file_exists($page_write_path)) {
            $page_write_path = Page::getPath();
        }
        ...
        $pageCode = (isset($page_path_absolute) && is_readable($page_path_absolute))
            ? file_get_contents($page_path_absolute)
            : get_page_val('page_code');
    }
    $is_page_writable = is_writable($page_write_path);
    $page_path = get_page_val('page_file_path');
    if ($page_path_absolute) {
    ?>
...
<?php } else {
    $pageCode = get_page_val('page_code');
                ...
                <textarea type="text" name="page_code" id="page_code" class="text-input resize-vertical r14"><?php echo safe_html($pageCode); ?></textarea>
    ...
    <div class="input-label">
        ...
        <div class="input-below"><?php _se('HTML %s attribute', '<a rel="external" href="https://www.w3schools.com/tags/att_a_rel.asp" target="_blank">rel</a>'); ?></div>
    </div>
    <div class="input-label">
        ...
        <ul data-content="dashboard-categories-list" class="tabbed-content-list table-li-hover table-li margin-top-20 margin-bottom-20">
            ...
            foreach (Handler::var('pages') as $k => $v) {
                ...
                <li>
                    ...
                    <span class="c13 display-table-cell padding-right-10"><a href="<?php echo get_base_url('dashboard/settings/pages/edit/' . $v['id']); ?>"><?php echo truncate(safe_html($v['title']), 64); ?></a></span>
                    <span class="display-table-cell"><a class="btn btn-small default" href="<?php echo get_base_url('dashboard/settings/pages/delete/' . $v['id'] . '/?auth_token=' . $auth_token); ?>" data-confirm="<?php _se("Do you really want to delete the page ID %s?", $v['id']); ?> <?php _se("This can't be undone."); ?>"><i class="fas fa-trash-alt margin-right-5"></i><?php _se('Delete'); ?></a></span>
                </li>
            <?php
            } // for each page
            ?>
